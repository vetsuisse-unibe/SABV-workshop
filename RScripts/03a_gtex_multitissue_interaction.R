#' ---
#' title: "Hands-on Exercise: GTEx Multi-Tissue Sex Interaction"
...
#' 
#' 1. identify the experimental unit in a repeated-measures study;
#' 2. interpret a sex × tissue interaction coefficient and interaction plot;
#' 3. distinguish a formal interaction test from overlap between subgroup gene lists;
#' 4. answer the same biological question using an explicit contrast.
#' 
#' Key terms used in this module:
#' 
#' - **Experimental unit:** the independent unit that was sampled — here, the donor, not each tissue tube.
#' - **Repeated measure:** another observation from the same experimental unit.
#' - **Blocking:** including donor identity in the model to account for stable donor-to-donor differences.
#' - **Interaction contrast:** the tissue effect in females minus the tissue effect in males.
#' - **limma-voom:** an RNA-seq workflow that estimates the count–variance relationship, then fits fast linear models.
#' 
#' ::: {.callout-note}
...
#' 
#' It was prepared from the public GTEx v8 files (`storage.googleapis.com/adult-gtex/…/v8/…`) — the same source **exercise 01** downloads — by keeping donors with *both* skeletal muscle and liver (paired design) and restricting to expressed genes. Here we just `load()` the file, then take a balanced-max subset (equal donors per sex).
#' 
#' **Version note.** Sample counts are release-specific. In v8 the paired muscle+liver set is **145 M / 61 F** donors, so the balanced-max subset is `min(145, 61) = 61` **per sex** (61 muscle + 61 liver each = 244 samples). A newer release such as **v11** has more donors and yields a larger balanced set (~**69** per sex) — the model and conclusions are unchanged, but pin to **v8** to reproduce the exact numbers here. The `sessionInfo()` printed at the end records the package versions used, so any downstream differences are easy to trace.
#' :::
#' 
#' # Overview
#' 
...
#' ```
#' 
#' Here, `subject_id` blocks donor-to-donor baseline differences. The `liver_female` indicator is the interaction term: it is 1 for female liver samples and 0 otherwise. It tests whether the liver-vs-muscle tissue effect differs in females compared with males.
#' 
#' We fit this model with **limma-voom** rather than a DESeq2 GLM: with 120+ donor parameters the two give equivalent inference, but limma-voom fits in **seconds** rather than many minutes — which matters for a live practical.
#' 
...
#' 
#' It carries a **scenario**, a **flawed design to critique**, and **specific tasks**. The SABV-preferred display for a sex × tissue effect is the **interaction plot** (Section 9): tissue on the x-axis, sex as separate lines, with SEM/CI error bars. The Venn diagram and heatmap here are deliberately included as **pitfall demonstrations**, not as evidence.
#' :::
#' 
#' ::: {.callout-note}
...
# Bioconductor: DESeq2 for the VST/PCA; edgeR + limma for the fast donor-blocked model.
...
# Donor-level table: one row per paired donor with their sex (used to build a balanced subset).
...
#' 
#' # 2. Create a Balanced Paired Workshop Subset
#' 
## -----------------------------------------------------------------------------
set.seed(42)

# Balanced-max: use every donor of the rarer sex and an equal random sample of the
# other, so the design stays sex-balanced (the workshop standard) while using the most
# donors possible. Here min(145 M, 61 F) = 61 per sex -> 122 donors, 244 samples.
target_per_sex <- min(table(paired_donors$sex))
selected_donors <- paired_donors %>%
  group_by(sex) %>%
  group_modify(~ dplyr::slice_sample(.x, n = min(target_per_sex, nrow(.x)))) %>%
  ungroup()

selected_meta <- meta_paired %>%
  dplyr::filter(SUBJID %in% selected_donors$SUBJID) %>%
  mutate(
...
print(sprintf("Selected paired samples: %d samples from %d donors", nrow(selected_meta), length(unique(selected_meta$SUBJID))))
...
# Align metadata rows to the count columns (data.frame for model.matrix / DESeq colData).
...
pca_data <- plotPCA(vsd, intgroup = c("tissue", "sex"), returnData = TRUE)
...
p_pca <- ggplot(pca_data, aes(x = PC1, y = PC2, color = tissue, shape = sex)) +
  ...
  labs(
    title = "PCA of Paired GTEx Muscle and Liver Samples",
...
#' 
## -----------------------------------------------------------------------------
# We fit the donor-blocked model with limma-voom. It gives equivalent inference to a
# DESeq2 GLM, but fits the 120+ donor parameters in seconds rather than many minutes —
# which is what makes it usable in a live practical.
#
# liver_female is the sex × tissue interaction term (a 2-level factor: 1 = female liver).

dge <- DGEList(counts = counts_mt)
dge <- calcNormFactors(dge)                       # TMM normalization

design_int <- model.matrix(~ subject_id + tissue + liver_female, data = selected_meta)
v   <- voom(dge, design_int)
fit <- eBayes(lmFit(v, design_int))

# The interaction coefficient (the liver_female factor -> column "liver_female1").
res_interaction_df <- topTable(fit, coef = "liver_female1", number = Inf, sort.by = "p")
res_interaction_df$gene_id <- rownames(res_interaction_df)
...
cat(sprintf("Sex × tissue interaction genes (FDR<0.05): %d of %d tested\n",
            length(interaction_hits), sum(!is.na(res_interaction_df$padj))))

cat("Top sex × tissue interaction genes:\n")
...
#' 
## -----------------------------------------------------------------------------
# We run the liver-vs-muscle comparison SEPARATELY within each sex (again with limma-voom,
# donor-blocked). This is a common paper-style shortcut: it is useful to look at, but
# comparing the two resulting gene lists is NOT a valid test of a sex-specific tissue
# effect (the interaction model above is). We show it here to expose the pitfall.

# --- Males only: liver vs muscle, paired within donor ---
males  <- selected_meta$sex == "Male"
meta_M <- droplevels(selected_meta[males, ])
dge_M  <- calcNormFactors(DGEList(counts_mt[, males]))
design_M <- model.matrix(~ subject_id + tissue, data = meta_M)
fit_M <- eBayes(lmFit(voom(dge_M, design_M), design_M))
res_tissue_M <- topTable(fit_M, coef = "tissueLiver", number = Inf)

# --- Females only (exactly the same steps) ---
females <- selected_meta$sex == "Female"
meta_F  <- droplevels(selected_meta[females, ])
dge_F   <- calcNormFactors(DGEList(counts_mt[, females]))
design_F <- model.matrix(~ subject_id + tissue, data = meta_F)
fit_F <- eBayes(lmFit(voom(dge_F, design_F), design_F))
res_tissue_F <- topTable(fit_F, coef = "tissueLiver", number = Inf)

male_tissue_hits   <- rownames(res_tissue_M)[res_tissue_M$adj.P.Val < 0.05]
female_tissue_hits <- rownames(res_tissue_F)[res_tissue_F$adj.P.Val < 0.05]
# interaction_hits was computed by the formal model in Section 5.
...
#' 
## -----------------------------------------------------------------------------
# log2 CPM (TMM-normalized) for display; row-centre so colour = deviation from the gene mean.
...
heatmap_mat <- logcpm[heatmap_gene_ids, , drop = FALSE]
...
#' 3. **Power.** Using the interaction-power logic from the liver module (Exercise 2.1b), comment on whether 61 donors/sex is likely to detect a modest sex × tissue interaction, and what an independent validation cohort would need.
