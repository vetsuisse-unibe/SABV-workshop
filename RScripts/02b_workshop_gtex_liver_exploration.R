...
#' 
#' Uses the GTEx liver subset prepared by tutorial 01 (`01_download_prepare_GTEx_liver`).
#' 
#' ## Learning objectives
#' 
#' By the end of this tutorial, you will be able to:
#' 
#' 1. assess whether one sex is consistently more variable in an RNA-seq dataset;
#' 2. fit and interpret a sex × age model;
#' 3. explain why pooling, separate subgroup analyses, and comparing subgroup p-values can mislead.
#' 
#' Key terms used in this module:
#' 
#' - **RNA-seq count:** the number of sequencing reads assigned to a gene.
#' - **VST (variance-stabilizing transformation):** a transformation that makes the spread of low- and high-expression genes more comparable.
#' - **FDR / Adjusted p-value (`padj`):** controls false positive rates when running thousands of simultaneous statistical tests.
#' - **Main effect:** the average association of one variable with expression.
#' - **Interaction:** occurs when the impact of one variable depends on the level of another.
#' - **Confounding:** overlapping relationships between variables that obscure individual effects unless explicitly controlled in the model.
#' 
...
#' ## Optional: a custom PCA plot and an age-confounding check
#' 
## -----------------------------------------------------------------------------
...
#' 
#' ### Quantifying Expression of X Chromosome Genes in Females and Males
#' 
...
#' 
#' **DESeq2 in one sentence:** for each gene, DESeq2 models read counts while accounting for differences in sequencing depth, then tests whether expression differs between groups.
#' 
## -----------------------------------------------------------------------------
#| fig-width: 12
#| fig-height: 6
...
#' 
#' The interaction is the key SABV test. It compares the age-related change in females with the age-related change in males. It is not obtained by comparing two separate p-values.
#' 
## -----------------------------------------------------------------------------
...
# Example genes from the top of res_int_df (ranked by interaction). Illustrative and
# selection-dependent, not independent confirmation (see caveats below the chunk).
# All three are FDR-significant for sexFemale.conditionOlder.
...
#' 
#' **The takeaway.** Don't mistake the `conditionOlder` coefficient for an overall age effect; it's strictly the effect of aging within whichever sex is set as the baseline. This explains why flipping the reference from females (−1.046) to males (−0.274) alters the coefficient without changing the underlying math.
#' 
#' A cleaner strategy is fitting a cell-means model without an intercept (`expression ~ 0 + group`). This directly estimates the average expression level for every sex × age group. Instead of navigating baseline reference levels, you simply subtract group means to extract whichever specific effect or interaction you want.
#' 
## -----------------------------------------------------------------------------
